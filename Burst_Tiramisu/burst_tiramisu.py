...
class GlobalMaxPool(torch.nn.Module):
    def __init__(self, func = torch.max):
        super(GlobalMaxPool, self).__init__()
        self.func = func
    def forward(self, input):
        #output = torch.max(input, dim=1)[0]

        if self.func == torch.mean:
            output = self.func(input, dim=1)
        else:
            output = self.func(input, dim=1)[0]
...
class BurstDenseUNet(BaseModel):
    """BurstDensUNet
    ...
    URL: https://openaccess.thecvf.com/content_ECCV_2018/papers/Miika_Aittala_Burst_Image_Deblurring_ECCV_2018_paper.pdf
    Notes:
        Coded with the help of https://github.com/bfortuner/pytorch_tiramisu
        MIT License - Copyright (c) 2018 Brendan Fortuner
        and https://github.com/keras-team/keras-contrib
        MIT License - Copyright (c) 2017 Fariz Rahman
        and the help of https://github.com/npielawski/pytorch_tiramisu
        MIT License - Copyright (c) 2020 Nicolas Pielawski
        and https://github.com/FrederikWarburg/Burst-Image-Deblurring
    """
    ...
    def __init__(
        ...
        efficient=False,
        nr_global_pooling_layers = None,
        global_pool_func = torch.max
    ):
        ...
        Args:
            ...
            nr_global_pooling_layers: The number of global max pooling layers to use in the encoder
        ...
        self.globalmaxpool = GlobalMaxPool(func = global_pool_func)
        self.nr_global_pooling_layers = (nr_global_pooling_layers if nr_global_pooling_layers else len(down_blocks))
        ...
        for block_size in self.down_blocks[:nr_global_pooling_layers]:
            ...
            skip_connections.insert(0, channels_count * 2)
            self.transitions_down.append(
                TransitionDown(
                    in_channels=channels_count * 2,
                    out_channels=int(channels_count * compression) * 2,
                    batch_norm=batch_norm,
                    dropout=dropout_rate,
                    pooling=transition_pooling,
                )
            )
            channels_count = int(channels_count * compression) * 2

        if self.nr_global_pooling_layers != len(self.down_blocks):
            for block_size in self.down_blocks[nr_global_pooling_layers:]:
                self.layers_down.append(
                    DenseBlock(
                        in_channels=channels_count,
        ...
        prev_block_channels = growth_rate * bottleneck_layers * 2 
    ...
    def forward(self, x):
        ...
        b, n_images, c, h_in, w_in = x.size()

        x = self.conv_init(x.view((b*n_images, c, h_in, w_in)))
        ...
        while i < len(self.down_blocks):
            if i < self.nr_global_pooling_layers:
                x = self.layers_down[i](x)
                _, _, h, w = x.size()
                
                x = x.view((b, n_images, -1, h, w))
                max_x = self.globalmaxpool(x)
                x = self.concat(x, max_x)
        ...
        x = self.globalmaxpool(x.view((b, n_images, -1, h_in, w_in)))
        x = torch.squeeze(x, 1)
